...
//		return (person.getAge() < Parameters.AGE_TO_LEAVE_HOME && person.getAtRiskOfPoverty() != null);		//Removes cases where gross earnings are not valid (i.e. are null, or negative)
		return (person.getDemAge() < Parameters.AGE_TO_BECOME_RESPONSIBLE && person.getBenefitUnit().getEquivalisedDisposableIncomeYearly() >= 0.);		//Removes cases where gross earnings are not valid (i.e. are null, or negative)
	}
