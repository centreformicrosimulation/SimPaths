...
public class ActivityAlignmentMacroShock implements IEvaluation {
    ...
    Map<Occupancy, MultiKeyCoefficientMap> coefficientMaps;
    Map<Occupancy, String> regressorsToModify;
    ...
    private void adjustEmployment(double newUtilityAdjustment) {
        for (Map.Entry<Occupancy, MultiKeyCoefficientMap> entry : coefficientMaps.entrySet()) {
            Occupancy occupancy = entry.getKey();
            MultiKeyCoefficientMap map = entry.getValue();
            String regressorToModify = regressorsToModify.get(occupancy);

            Object currentValueObj = map.getValue(regressorToModify);
            if (!(currentValueObj instanceof Number)) {
                throw new IllegalArgumentException("Expected a numeric value for key: " + regressorToModify);
            }

            double currentValue = ((Number) currentValueObj).doubleValue();
            double newValue = currentValue + newUtilityAdjustment;
            map.replaceValue(regressorToModify, newValue);
        }
