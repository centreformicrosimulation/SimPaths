...
| `InitialPopulations/compile/RegressionEstimates/` | 17 Stata scripts that estimate regression coefficients and produce the `reg_*.xlsx` files. The 14 `reg_*.do` scripts each estimate one behavioural model; `programs.do` defines shared utility functions used by the estimation scripts; `variable_update.do` prepares variables before estimation; `master.do` orchestrates the full run. |
