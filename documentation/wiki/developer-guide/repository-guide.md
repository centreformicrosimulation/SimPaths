...
For a description of the variables in output CSV files, see the [SimPaths Variable Codebook workbook on GitHub](https://github.com/simpaths/SimPaths/blob/develop/documentation/SimPaths_Variable_Codebook.xlsx). For a description of each `reg_*`, `align_*`, and `scenario_*` input file, see the [Model Parameterisation page](../overview/parameterisation.md).
